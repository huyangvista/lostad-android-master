...
import com.lostad.app.demo.view.mainFragment.joblog.JobLogFragment;
...
public class IntegrationFragment2 extends BaseFragment {
	...
	private void initTabs(final LayoutInflater inflater) {
		...
		for (String name : mListTabNames) {
			if(name.equals("Tab1")){
				pages.add(FragmentPagerItem.of(name, JobLogFragment.class));   //加载每个

			}else{
				pages.add(FragmentPagerItem.of(name, ListWaterFragment.class));   //加载每个
